...
ConclusionKind = Literal["NeedSync", "AlreadyIncluded", "ManualReview", "OutOfScope"]
...
NEED_SYNC_TARGET_TYPES_WITH_DEVELOP = frozenset({"release", "fix", "develop"})
...
class CommitAnalysis:
    ...
    has_high_priority_rule: bool = False
...
class TargetSnapshot:
    ...
    develop_backfill_allowed: bool = True
    cross_product_linked: bool = False
...
def _sha_matches(candidate: str, source_sha: str) -> bool:
    ...
    return candidate == source_sha or source_sha.startswith(candidate) or candidate.startswith(source_sha)
...
def _target_role_eligible(
    ...
    *,
    ...
    if target.branch_type in INELIGIBLE_TARGET_TYPES:
        return False
    allowed = (
        NEED_SYNC_TARGET_TYPES_WITH_DEVELOP
        if develop_backfill_enabled and target.develop_backfill_allowed
        else NEED_SYNC_TARGET_TYPES
    )
    return target.branch_type in allowed


def _lifecycle_allows(source: CommitAnalysis, target: TargetSnapshot) -> bool:
    if target.lifecycle == "eol":
        return False
    if target.lifecycle == "preview":
        return False
    if target.lifecycle == "frozen":
        return source.has_high_priority_rule
    return True
...
def _need_sync_confidence(source: CommitAnalysis, target: TargetSnapshot) -> Confidence:
    if source.recognition_source == "heuristic:files+functions" and not source.issue_ids:
        if target.fix_clearly_missing:
            return "low"
...
def conclude_pair(
    ...
    *,
    ...
    similarity_evidence = _check_high_similarity(
        target,
    ...
    if not target.in_same_homologous_set and not target.cross_product_linked:
        return Conclusion(
            kind="OutOfScope",
    ...
    if not _lifecycle_allows(source, target):
        return Conclusion(
            kind="OutOfScope",
            evidence=[f"目标分支生命周期 {target.lifecycle} 不纳入自动需要同步。"],
            confidence="high",
        )

    if not _target_role_eligible(target, develop_backfill_enabled=develop_backfill_enabled):
        return Conclusion(
            kind="OutOfScope",
    ...
    matched_files = [path for path in source.changed_files if path in target.files_on_target]
    if matched_files:
