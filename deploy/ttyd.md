...
停止 bsa-web **不会**回收已 spawn 的 ttyd：`start_new_session=True` 使 ttyd 处于
独立进程组/会话，平台进程退出后其仍存活。残留 ttyd 由自身两层机制自回收：
- `timeout 1800`：会话空闲 30 分钟强制结束命令树；
- `-o`：单客户端，客户端断开即退出。

暴露面有限：ttyd 仅 `127.0.0.1` 随机端口监听，平台退出后无 WS 反代承接，外网
不可达，仅本机存留，且进程权限等同平台账号（bsa）。如需立即清理可手动
`pkill -f '^ttyd -o -p 0'`（ttyd 为唯一以该签名运行的进程）；也可在平台启动时
加一次孤儿 ttyd sweep（当前未实现，如需再加）。
