...
叠加按状态/权限显示的操作入口：
- SUCCESS（且为最近完成周期）→ 推送（四道闸 + 确认框，复用 _push_confirm.html）；
...
由 WebSSH 任务（task 5）实现受限终端；推送仅对最近完成周期开放（超期分支无推送入口）。
"""
...
def task_detail(
    ...
    # 推送仅对最近完成周期的 SUCCESS 现场开放（超期分支无推送入口）
    ...
    return _render(
        ...
        show_push=not branch_abandoned and is_current and status == "SUCCESS",
    )
