...
# 投影 commit 记录 → 富化素材：按 node 分派可展示的「错误/原因/日志/diff」。
# build/fix_build 的每次尝试都覆盖投影同一 build[model]（引擎只存终态），无法从
# 投影还原「失败→修复→通过」的过程——那由 progress.jsonl 逐次 start/end 忠实记录。
# 本函数把投影里的失败归因与处理产物挂到对应的最后一次尝试步骤上供展开。
def _attach(
...
def _enrich_steps(steps: list[dict], branch: dict | None) -> None:
    ...
    # model 标签：commit build / baseline 的展示字段已由 enrich_build_outcomes 补
    for s in steps:
...
def task_detail(
    ...
    # 处理过程时间线：读该周期 progress.jsonl，保留本分支步骤 + 周期级步骤
    # （detect/report 等无 target），并把投影里的处理产物富化挂到对应步骤展开。
    steps = []
    for s in read_progress(settings.log_dir, cycle_id):
        if not s.get("target") or s.get("target") == target:
            steps.append(s)
    _enrich_steps(steps, branch)
