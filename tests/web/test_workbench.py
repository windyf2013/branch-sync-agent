...
from bsa_web.projection import (
...
def _payload(*, status="REPORTED", branch_results=None, action_required=None, detected_commits=None, scan_window=None):
    return {
        "cycle_id": "cycle-2026-08-20",
        "status": status,
        "scan_window": scan_window or ["", ""],
        "detected_commits": detected_commits or [],
        "decisions": {},
...
class TestWorkbenchView:
    def test_manual_tasks_cutoff_is_cycle_started_at_not_window_start(self, tmp_path, monkeypatch):
        # 回归：手动任务保留边界 = 最近周期启动时刻（cycle.json started_at），
        # 而非扫描窗口起点。8/30T15:00Z 在旧边界（窗口起点 14:00Z）之内、但在
        # 新边界（周期启动 08-31T00:00+08:00 = 08-30T16:00Z）之前 → 必须淡出面板。
        log_dir = tmp_path
        _write_cycle_record(
            log_dir, "cycle-2026-08-31", "REPORTED", "2026-08-31T00:00:00+08:00"
        )
        app = _make_app(tmp_path)
        client = _client(app)
        _login(client)
        payload = _payload(scan_window=["2026-08-30T22:00:00+08:00", "2026-08-31T22:00:00+08:00"])
        monkeypatch.setattr(
            "bsa_web.projection.latest_completed_cycle", lambda log_dir: "cycle-2026-08-31"
        )
        monkeypatch.setattr(
            "bsa_web.projection.load_cycle", lambda log_dir, cycle_id: payload
        )
        # A：旧边界保留、新边界过滤；B：新旧边界都保留。
        _add_task(app, target="feat/a", state="succeeded",
                  created_at="2026-08-30T15:00:00+00:00")
        _add_task(app, target="feat/b", state="succeeded",
                  created_at="2026-08-31T02:00:00+00:00")
        r = client.get("/")
        assert r.status_code == 200
        assert "feat/b" in r.text
        assert "feat/a" not in r.text
    ...
    def test_workbench_empty_branch_results_shows_cycle_summary_row(self, tmp_path, monkeypatch):
        # 空 branch_results 且有待确认项时：任务表仍渲染周期级摘要行（含待确认计数）。
        client = _client(_make_app(tmp_path))
        _login(client)
        detected = [
            {"sha": f"c{i}", "message": f"m{i}", "author": "dev",
             "committed_at": "2026-08-20T10:00:00+08:00", "changed_files": []}
            for i in range(8)
        ]
        payload = _payload(
            status="SUCCESS",
            ...
            action_required=[
                {"sha": f"c{i}", "kind": "ManualReview", "reason": "存疑"}
                for i in range(2, 6)
            ],
        )
        monkeypatch.setattr(
        ...
        r = client.get("/")
        assert r.status_code == 200
        assert "task-row-cycle-summary" in r.text
        assert "/cycle/cycle-2026-08-20" in r.text
        assert "检测 8" in r.text
        assert "待确认 4" in r.text
        assert "本周期无可同步任务" in r.text
        assert "当前无任务记录" not in r.text
    ...
    def test_workbench_success_with_pending_shows_review_badge(self, tmp_path, monkeypatch):
        # SUCCESS 但存在待人工项 → period-bar 与状态徽章显示「完成 · N 待人工」，
        # 不只用纯「完成」掩盖「同步 0 + 待人工 N」的真相。
        client = _client(_make_app(tmp_path))
        _login(client)
        payload = _payload(
            status="SUCCESS",
        ...
        assert r.status_code == 200
        assert "完成" in r.text
        assert "period-review-badge" in r.text
        assert "4 待人工" in r.text

    def test_workbench_success_no_pending_shows_plain_done(self, tmp_path, monkeypatch):
        # SUCCESS 且无待人工 → 维持纯「完成」，不追加待人工标记。
        client = _client(_make_app(tmp_path))
        _login(client)
        ...
        r = client.get("/")
        assert r.status_code == 200
        assert "period-review-badge" not in r.text

    def test_workbench_review_kpi_links_to_cycle(self, tmp_path, monkeypatch):
        # 待确认 KPI 可点 → 落到周期概览，不再是纯 <div>。
        client = _client(_make_app(tmp_path))
        _login(client)
        payload = _payload(
            status="SUCCESS",
        ...
        r = client.get("/")
        assert r.status_code == 200
        # 待确认 KPI 卡渲染为链接，指向周期概览
        assert 'data-kpi-review-link' in r.text or 'href="/cycle/cycle-2026-08-20"' in r.text
...
class TestProjection:
    ...
    def test_latest_cycle_start_utc_aware_normalized(self, tmp_path):
        # tz-aware started_at → 直接按自身时区转 UTC-naive，环境无关。
        log_dir = tmp_path / "logs"
        _write_cycle_record(
            log_dir, "cycle-2026-08-31", "REPORTED", "2026-08-31T16:00:04+00:00"
        )
        assert latest_cycle_start(str(log_dir)) == datetime(2026, 8, 31, 16, 0, 4)

    def test_latest_cycle_start_naive_local_normalized(self, tmp_path):
        # naive 本地墙钟（引擎真实格式）→ 按系统本地时区转 UTC-naive。
        # 期望值用同一机制计算，避免硬编码 +08:00（机器无关）。
        log_dir = tmp_path / "logs"
        _write_cycle_record(log_dir, "cycle-2026-09-01", "REPORTED", "2026-09-01T00:00:04")
        expected = (
            datetime.fromisoformat("2026-09-01T00:00:04")
            .astimezone()
            .astimezone(timezone.utc)
            .replace(tzinfo=None)
        )
        assert latest_cycle_start(str(log_dir)) == expected

    def test_latest_cycle_start_picks_recent_non_running(self, tmp_path):
        log_dir = tmp_path / "logs"
        _write_cycle_record(log_dir, "cycle-2026-08-20", "REPORTED", "2026-08-20T22:00:00+00:00")
        _write_cycle_record(log_dir, "cycle-2026-08-24", "running", "2026-08-24T22:00:00+00:00")
        assert latest_cycle_start(str(log_dir)) == datetime(2026, 8, 20, 22, 0, 0)

    def test_latest_cycle_start_none_when_only_running(self, tmp_path):
        log_dir = tmp_path / "logs"
        _write_cycle_record(log_dir, "cycle-2026-08-24", "running", "2026-08-24T22:00:00+00:00")
        assert latest_cycle_start(str(log_dir)) is None

    def test_latest_cycle_start_none_when_empty(self, tmp_path):
        assert latest_cycle_start(str(tmp_path)) is None
